...
def run_stimulus(word):
    ...
    lex_decision.goals["imaginal"].add(actr.makechunk(nameofchunk='start',
                                                      typename="word"))
    lex_decision.goals["imaginal"].delay = 0
...
with lex_decision_with_bayes:
    ...
    #step = Metropolis()
    #db = SQLite('lex_dec_pyactr_chain_with_imaginal_delay_0.sqlite')
    #trace = sample(draws=60000, trace=db, njobs=1, step=step, init='auto')

with lex_decision_with_bayes:
    trace = load('./data/lex_dec_pyactr_chain_with_imaginal_delay_0.sqlite')
    trace = trace[10500:]
...
def generate_lex_dec_pyactr_with_imaginal_delay_0_figure():
    ...
    ax1.plot(np.linspace(500, 800, 10), np.linspace(500, 800, 10),\
             color='red', linestyle=':')
    ax1.set_title('Lex. dec. model (pyactr, with imaginal, delay 0): RTs')
    ...
    ax2.plot(np.linspace(50, 100, 10)/100,\
             ...
             color='red', linestyle=':')
    ax2.set_title('Lex. dec. model (pyactr, with imaginal, delay 0): Prob.s')
