...
class Environment(env.Environment): #subclass Environment
    """
    Example of environment used for ACT-R model. This model changes display based on run_time. You can run this in u2_motormodel in the folder tutorials.
    """

    def __init__(self):
        self.text = "a b c d".split()
        self.run_time = 0.4
    ...
    def environment_process(self, start_time):
        ...
        for letter in self.text: #run through letters, print them, yield a corresponding event
            self.output(letter) #output on environment
